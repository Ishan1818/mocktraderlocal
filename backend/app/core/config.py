...
class Settings(BaseSettings):
    ...
    supabase_leaderboard_table: str = Field(
        default="participant_snapshots", validation_alias="SUPABASE_LEADERBOARD_TABLE"
    )
    leaderboard_sync_interval_sec: float = Field(
        default=45.0, validation_alias="LEADERBOARD_SYNC_INTERVAL_SEC"
    )
    participant_session_id: str | None = Field(default=None, validation_alias="PARTICIPANT_SESSION_ID")
    serve_static_ui: bool = Field(default=False, validation_alias="SERVE_STATIC_UI")
    hide_admin_ui: bool = Field(default=False, validation_alias="HIDE_ADMIN_UI")
    timeline_decrypt_key: str | None = Field(default=None, validation_alias="TIMELINE_DECRYPT_KEY")
    organizer_passkey: str = Field(default="finclub123", validation_alias="ORGANIZER_PASSKEY")
    supabase_event_control_table: str = Field(
        default="event_control", validation_alias="SUPABASE_EVENT_CONTROL_TABLE"
    )
    ...
    def database_url(self) -> str:
        ...
        if self.local_instance_mode:
            # Offline mode: always use a persistent per-user DB (ignore stray :memory: test URLs).
            if url and ":memory:" not in url:
                ...
                return url
            path = self.default_local_db_path.replace("\\", "/")
