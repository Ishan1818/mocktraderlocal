...
def allot_ipo(db: Session, ipo_id: int, *, seed: int | None = None) -> dict:
    ...
    if settings.local_instance_mode:
        return allot_ipo_personal(db, ipo_id)
    ipo = db.get(IPO, ipo_id)
    if ipo is None:
        raise IPOServiceError("ipo not found")
...
def _personal_allot_seed(trader: Trader, ipo_id: int) -> int:
    settings = get_settings()
    session = trader.session_id or settings.participant_session_id or str(trader.id)
    digest = hashlib.sha256(f"{trader.name}:{session}:{ipo_id}".encode()).hexdigest()
    return int(digest[:16], 16)


def allot_ipo_personal(db: Session, ipo_id: int) -> dict:
    """Per-participant probabilistic lottery — each requested lot drawn independently."""
    ipo = db.get(IPO, ipo_id)
    if ipo is None:
        raise IPOServiceError("ipo not found")
    if ipo.status not in {IPOStatus.CLOSED, IPOStatus.OPEN}:
    ...
    win_rate = float(ipo.winning_lots) / float(ipo.total_lots) if ipo.total_lots else 0.0
    ...
    for app in apps:
        ...
        if trader is None:
            continue
        rng = random.Random(_personal_allot_seed(trader, ipo.id))
        lots = 0
        for _ in range(app.requested_lots):
            if lots >= ipo.maximum_lots_per_user:
                break
    ...
    ipo.status = IPOStatus.ALLOTTED
    db.commit()
    return {"ipo_id": ipo.id, "allocations": results, "allotment_mode": "personal"}
