...
def bootstrap_universe(db: Session) -> dict:
    ...
    if settings.local_instance_mode:
        from app.seed.universe_loader import (
            apply_universe_simulation_settings,
            canonical_tradable_count,
            seed_sectors_from_json,
            seed_stocks_from_json,
        )

        sectors_created = seed_sectors_from_json(db)
        stocks_created = seed_stocks_from_json(db)
        apply_universe_simulation_settings(db)
        expected = canonical_tradable_count()
    else:
        from app.seed.tradeverse_stocks import canonical_tradable_count, seed_tradeverse_stocks
...
def _canonical_tickers() -> set[str]:
    settings = get_settings()
    if settings.local_instance_mode:
        from app.seed.universe_loader import canonical_tradable_tickers

        return set(canonical_tradable_tickers())
